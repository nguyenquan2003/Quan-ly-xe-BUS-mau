...
var mgr = null;
var disableBusStyles = null;
var styledMapOptions = null;
var testmap = null;
var batch1 = []; //Mang Marker
var batch2 = [];
var batch3 = [];
var batch4 = [];
var batch5 = [];
...
//    testmap = new GMapType(disableBusStyles, styledMapOptions);
...
//        });
    ...
    //    myEventListeners = new fListeners();
        ...
        for(i=0;i<result.Points.length;i++)
        {
            ...
            var marker = CreateMarker(myPoint, myIcon_google, result.Points[i].InfoHTML, result.Points[i].Draggable, result.Points[i].ToolTip, result.Points[i].Higtlight);
            marker.value = result.Points[i].ID;
            markers.pushValue(marker);
            if (result.Points[i].MinZoom == 14) {
                batch1.push(marker);
            }
            if (result.Points[i].MinZoom == 15) {
                batch2.push(marker);
            }
            if (result.Points[i].MinZoom == 16) {
                batch3.push(marker);
            }
            if (result.Points[i].MinZoom == 17) {
                batch4.push(marker);
            }
            if (result.Points[i].MinZoom == 18) {
                batch5.push(marker);
            }
//            mgr.addMarker(marker, result.Points[i].MinZoom);
//            mgr.refresh();

            //alert(result.Points[i].ID);
            //map.addOverlay(markers.getLastValue());
        }
        mgr.addMarkers(batch1, 14);
        mgr.addMarkers(batch2, 15);
        mgr.addMarkers(batch3, 16);
        mgr.addMarkers(batch4, 17);
        mgr.addMarkers(batch5, 18);
        mgr.refresh();
    ...
    if (result.Directions.Addresses.length > 0) {
        if (result.Directions.ShowDirectionInstructions) {
            //Direction
            gdir = new GDirections(map, document.getElementById("d"));
            }
            else {
            ...
            //gdir.load("from: " + result.Directions.Addresses[0] + " to: " + result.Directions.Addresses[1], { "locale": result.Directions.Locale });
                ...
                gdir.load(sDir, { "locale": result.Directions.Locale });
            }
    }
    ...
    for(i=0;i<result.Points.length;i++)
    {
        ...
        if(result.Points[i].PointStatus=='C')
        {
            ...
            if(marker!=null)
            {
                ...
                map.removeOverlay(marker); // thu nghiem manager marker
            ...
            markers.pushValue(marker);
//            if (result.Points[i].MinZoom == 14) {
//                batch1_1.push(marker);
//            }
//            if (result.Points[i].MinZoom == 15) {
//                batch2_2.push(marker);
//            }
//            if (result.Points[i].MinZoom == 16) {
//                batch3_3.push(marker);
//            }
//            if (result.Points[i].MinZoom == 17) {
//                batch4_4.push(marker);
//            }
//            if (result.Points[i].MinZoom == 18) {
//                batch5_5.push(marker);
//            }
...
function CreateMarker(point, icon1, InfoHTML, bDraggable, sTitle, hightLigth) {
    var marker;
    ...
    if (hightLigth == true) {
        GEvent.addListener(marker, "mouseover", function () {
            this.openInfoWindowHtml(InfoHTML); RaiseEvent('PushpinClicked', this.value);
